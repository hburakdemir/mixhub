...
import './Hemsirelik.css';

export default function HemsirelikView() {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const response = await fetch('http://localhost:5000/api/posts?department=Hemsirelik');
        if (!response.ok) {
  ...
  return (
    <div className="department-container">
      <div className="posts-section">
        <h1 className="department-title">Hemşirelik Fakültesi</h1>
        {posts.length === 0 ? (
