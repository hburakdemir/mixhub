...
import './GuzelSanatlar.css';

export default function GuzelSanatlarView() {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const response = await fetch('http://localhost:5000/api/posts?department=GuzelSanatlar');
        if (!response.ok) {
  ...
  return (
    <div className="department-container">
      <div className="posts-section">
        <h1 className="department-title">Güzel Sanatlar Fakültesi</h1>
        {posts.length === 0 ? (
