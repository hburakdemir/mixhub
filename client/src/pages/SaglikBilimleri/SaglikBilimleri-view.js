...
import './SaglikBilimleri.css';

export default function SaglikBilimleriView() {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const response = await fetch('http://localhost:5000/api/posts?department=SaglikBilimleri');
        if (!response.ok) {
  ...
  return (
    <div className="department-container">
      <div className="posts-section">
        <h1 className="department-title">Sağlık Bilimleri Fakültesi</h1>
        {posts.length === 0 ? (
