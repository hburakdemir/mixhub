...
import './FizikTedavi.css';

export default function FizikTedaviView() {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const response = await fetch('http://localhost:5000/api/posts?department=FizikTedavi');
        if (!response.ok) {
  ...
  return (
    <div className="department-container">
      <div className="posts-section">
        <h1 className="department-title">Fizik Tedavi ve Rehabilitasyon Fakültesi</h1>
        {posts.length === 0 ? (
