...
import './IktisadiIdari.css';

export default function IktisadiIdariView() {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const response = await fetch('http://localhost:5000/api/posts?department=IktisadiIdari');
        if (!response.ok) {
  ...
  return (
    <div className="department-container">
      <div className="posts-section">
        <h1 className="department-title">İktisadi ve İdari Bilimler Fakültesi</h1>
        {posts.length === 0 ? (
