...
import './AlternatifEnerji.css';

export default function AlternatifEnerjiView() {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const response = await fetch('http://localhost:5000/api/posts?department=AlternatifEnerji');
        if (!response.ok) {
  ...
  return (
    <div className="department-container">
      <div className="posts-section">
        <h1 className="department-title">Alternatif Enerji ve Yenilenebilir Kaynaklar</h1>
        {posts.length === 0 ? (
