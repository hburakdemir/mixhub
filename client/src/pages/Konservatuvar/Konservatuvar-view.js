...
import './Konservatuvar.css';

export default function KonservatuvarView() {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const response = await fetch('http://localhost:5000/api/posts?department=Konservatuvar');
        if (!response.ok) {
  ...
  return (
    <div className="department-container">
      <div className="posts-section">
        <h1 className="department-title">Konservatuvar</h1>
        {posts.length === 0 ? (
