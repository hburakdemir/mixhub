...
import './EndustriUrunleri.css';

export default function EndustriUrunleriView() {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const response = await fetch('http://localhost:5000/api/posts?department=EndustriUrunleri');
        if (!response.ok) {
  ...
  return (
    <div className="department-container">
      <div className="posts-section">
        <h1 className="department-title">Endüstri Ürünleri Tasarımı</h1>
        {posts.length === 0 ? (
