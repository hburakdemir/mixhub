...
import './DisHekimligi.css';

export default function DisHekimligiView() {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const response = await fetch('http://localhost:5000/api/posts?department=DisHekimligi');
        if (!response.ok) {
  ...
  return (
    <div className="department-container">
      <div className="posts-section">
        <h1 className="department-title">Diş Hekimliği Fakültesi</h1>
        {posts.length === 0 ? (
