...
import './ElektrikElektronik.css';

export default function ElektrikElektronikView() {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const response = await fetch('http://localhost:5000/api/posts?department=ElektrikElektronik');
        if (!response.ok) {
  ...
  return (
    <div className="department-container">
      <div className="posts-section">
        <h1 className="department-title">Elektrik-Elektronik Mühendisliği</h1>
        {posts.length === 0 ? (
