...
import './YabanciDiller.css';

export default function YabanciDillerView() {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const response = await fetch('http://localhost:5000/api/posts?department=YabanciDiller');
        if (!response.ok) {
  ...
  return (
    <div className="department-container">
      <div className="posts-section">
        <h1 className="department-title">Yabancı Diller Yüksekokulu</h1>
        {posts.length === 0 ? (
